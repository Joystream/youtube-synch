...
export class Result<T, TE extends DomainError> {
  ...
  static bindAsync<T, K, TE extends DomainError>(
    res: Result<T, TE>,
    f: (value: T) => Promise<Result<K, TE>>
  ): Promise<Result<K, TE>> {
    if (res.isFailure) return Promise.resolve(Result.Error<K, TE>(res.error))
    return f(res.value)
  }

  static async checkAsync<T, K, TE extends DomainError>(
    res: Result<T, TE>,
    f: (value: T) => Promise<Result<K, TE>>
  ): Promise<Result<T, TE>> {
    if (res.isFailure) return res
    const secondResult = await f(res.value)
    if (secondResult.isSuccess) return res
    return Result.Error<T, TE>(secondResult.error)
  }

  static async concat<T, K, TE extends DomainError>(
    res: Result<T, TE>,
    f: (value: T) => Promise<Result<K, TE>>
  ): Promise<Result<[T, K], TE>> {
    if (res.isFailure) return Result.Error<[T, K], TE>(res.error)

    const nextResult = await f(res.value)
    return nextResult.map((nextValue) => [res.value, nextValue] as [T, K])
  }

  static ensure<T, TE extends DomainError>(
    res: Result<T, TE>,
    f: (value: T) => boolean,
    fe: (value: T) => TE
  ): Result<T, TE> {
    if (res.isSuccess && f(res.value)) return res
    if (res.isFailure) return res
  ...
  async tapAsync<K>(f: (value: T) => Promise<K>): Promise<Result<T, TE>> {
    if (this.isSuccess) await f(this.value)
    return this
  }

  map<K>(mapper: (value: T) => K): Result<K, TE> {
    if (this.isSuccess) return Result.Success(mapper(this.value))
    return Result.Error(this.error)
  }

  onFailure(err: (error: TE) => void) {
    if (this.isFailure) err(this.error)
    return this
  }

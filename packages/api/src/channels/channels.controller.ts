...
export class ChannelsController {
  ...
  @ApiOperation({
    description:
      'Updates given channel. Note: only `shouldBeInjested` is available for update at the moment',
  })
  ...
  @ApiOperation({
    description:
      'Retrieves already ingested(spotted on youtube and saved to the database) videos for a given channel.',
  })
  @ApiResponse({ type: VideoDto, isArray: true })
  @Get(':id/videos')
  async getVideos(@Param('userId') userId: string, @Param('id') id: string) {
    const result = await this.videosRepository.query({ channelId: id }, (q) =>
      q.sort('descending')
    )
    if (result.isSuccess) return result.value
    throw new HttpException(result.error, 500)

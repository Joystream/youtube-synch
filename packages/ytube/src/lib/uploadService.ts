...
export class UploadService{
    ...
    async uploadVideo(channelId: string, videoId: string):Promise<Video>{
        ...
        await repo.update({id: video.id, channelId: video.channelId, state: 'uploadToJoystreamStarted'})
        ...
        return await upload
            .promise()
            .then(up => repo.update({id: video.id, channelId: video.channelId, state: 'uploadToJoystreamSucceded', destinationUrl: up.Location}))
            .catch(err =>{
                console.log(err);
                return repo.update({id: video.id, channelId: video.channelId, state: 'uploadToJoystreamFailed'})
            })
